...
void AudioSessionList::removeExpiredSessions()
{
	for (auto it = m_audioSessions.begin(); it != m_audioSessions.end(); ) {
		if (it->isExpired()) {
			spdlog::debug("Audio session for \"{}\" (PID {}) expired. Going to remove it from the list", it->getRelatedProcessName(), it->getRelatedPID());
...
{
	spdlog::debug("Trying to add audio session \"{}\" (PID {})...", newAudioSession.getRelatedProcessName(), newAudioSession.getRelatedPID());

	if (isPidAlreadyInList(newAudioSession.getRelatedPID())) {
		spdlog::debug("New audio session related to PID {} is already in the list", newAudioSession.getRelatedPID());
	}
	else {
...
void AudioSessionList::printAllAudioSessionsInfo()
{
	...
	for (const auto& session : m_audioSessions) {
		...
		spdlog::info("{}. PID {} -> {} {}", sessionCnt, session.getRelatedPID(), session.getRelatedProcessName(), isExpired);
